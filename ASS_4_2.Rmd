...
# DET
t.test(crqa_df_hr$DET, crqa_df_resp_shuf$DET)
t.test(crqa_df_resp$DET, crqa_df_resp_shuf$DET) 

# L
t.test(crqa_df_hr$L, crqa_df_resp_shuf$L)
t.test(crqa_df_resp$L, crqa_df_resp_shuf$L) 

# Lmax
t.test(crqa_df_hr$LMAX, crqa_df_resp_shuf$LMAX)
t.test(crqa_df_resp$LMAX, crqa_df_resp_shuf$LMAX) 
```
...
```{r Collecting df's}
# Creating columns in order to gather data later
...
#write.csv(all_crqa_hr, file = "df_hr.csv")
#write.csv(all_crqa_resp, file = "df_resp.csv")
```

  3. Statistic comparison between the data and surrogate pairs
```{r T-tests: RR}
## T-test: HR comparison of regular to surrogate
t.test(crqa_df_hr$RR, surPair_rqa$RR.HR)
# t = -0.5735, df = 32.025, p-value = 0.5703 
#alternative hypothesis: true difference in means is not equal to 0
#95 percent confidence interval:
# -4.174809  2.340385
#sample estimates:
...
## T-test: Respiration comparison of regular to surrogate
t.test(crqa_df_resp$RR, surPair_rqa$RR.Resp) 
#t = 0.75387, df = 45.673, p-value = 0.4548
#alternative hypothesis: true difference in means is not equal to 0
#95 percent confidence interval:
# -0.2708106  0.5950205
#sample estimates:
...
# DET
t.test(crqa_df_hr$DET, surPair_rqa$DET.HR)
t.test(crqa_df_resp$DET, surPair_rqa$DET.Resp) 
 
# L
t.test(crqa_df_hr$L, surPair_rqa$L.HR)
t.test(crqa_df_resp$L, surPair_rqa$L.Resp) 

# Lmax
t.test(crqa_df_hr$LMAX, surPair_rqa$maxL.HR)
t.test(crqa_df_resp$LMAX, surPair_rqa$maxL.Resp) 

# NO difference between real and surrogate in different measures --> surrogate 
``` 


  Creating models for statistical comparison between the OG, shuffled and surrogate pairs
```{r Models for statistic comparison: Shuffled + surrogate}

model1 <- lm(RR~Comparison, all_crqa_hr)
summary(model1)

```


NOT USING THIS (underneath)
```{r Summarise function}

royalewithcheese = function(df){
summarised_df = df  %>%
  group_by(group_trial) %>% 
...
sum_og_df = royalewithcheese(crqa_df_hr)
sum_shuf_df = royalewithcheese(shuf_df)
...
- CRQA needs parameters: delay, radius
